...
contratomovilduo *cmd1 = new contratomovilduo(75547001,f1,0.12, 0.09 ,110,"DANES");

        this->contratos[this->ncon] = cmd1;
        this->ncon++;
...
for(int j=0;j<ncon;j++){
 ...
 void Empresa::mejoresclientestp(){
 float mayor_factura = 0;
 int contador =0;
 float sumatorio=0;
 long int dnicli;
 ...
 dnicli = this->clientes[j]->getDNI();

 for(int i=0; i<this->ncon; i++){
        if((typeid(*contratos[i]) == typeid(contratotp)) && (dnicli == this->contratos[i]->getdnicontrato()) ){ //(this->contratos[i]->factura() > mayor_factura)
            sumatorio =  this->contratos[i]->factura() + sumatorio;
            if (sumatorio > mayor_factura)
            mayor_factura = sumatorio;
 }
 }
}
//Ya tenemos la mayor factura
 for(int j =0;j<this->ncli; j++){
...
cout << "Hay " << contador << " clientes de TP cuyo sumatorio de facturas es: " << mayor_factura << "$\n\nSus nombres son:\n\n";
...
for(int j =0;j<this->ncli; j++){
 ...
 //MOD 3b)
int Empresa::multanuevosmovil(){

int mayor_anio = 0;
int n=0;
    for(int i=0; i<ncli; i++)
        if(Empresa::clientes[i]->getfecha().getanio() > mayor_anio)
            mayor_anio = Empresa::clientes[i]->getfecha().getanio();
     ...
     for (int i=0; i< this->ncon; i++) {
        ...
        if(cm){ //El casting se ha hecho bien porque el i-esimo es un contrato movil
                if(cm->getfechacontrato()->getanio() == mayor_anio){
            cm->setPrecioMinuto(cm->getPrecioMinuto() + 0.10);
            n++;
        }
        }
